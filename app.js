...
let state = {
  ...
  selectedItem: null, // { full_id, display_name, kind } — the catalogue item currently "held", waiting for a slot click
};
...
async function apiListItems({ search, page, limit }) {
  ...
  params.set("namespace", "minecraft");
...
function renderCatalogue() {
  ...
  if (state.selectedItem) {
    ...
    targetLabel.innerHTML = `<span>✋ Holding <b>${escapeHtml(state.selectedItem.display_name)}</b> — click a slot to place it</span><span class="clear-target" id="clearTargetBtn">cancel</span>`;
  } else {
  ...
  for (const entry of c.items) {
    ...
    const isSelected = state.selectedItem && state.selectedItem.full_id === entry.full_id && state.selectedItem.kind === entry.kind;
    slot.className = "cat-slot" + (isSelected ? " selected" : "");
    slot.draggable = true;
    ...
    // Drag-and-drop remains available as an alternative to click-then-click.
...
function handleCatalogueClick(entry) {
  ...
  // Clicking the already-held item again puts it back down (deselect).
  if (state.selectedItem && state.selectedItem.full_id === payload.full_id && state.selectedItem.kind === payload.kind) {
...
function makeSlotEl({ filled, slotKind, slotIndex, recipeId, isOutput, isFuel, showCount }) {
  const slot = document.createElement("div");
  const awaiting = !!state.selectedItem;
  slot.className = "slot" + (filled ? " filled" : "") + (isOutput ? " output-slot" : "") + (isFuel ? " fuel-slot" : "") + (awaiting ? " awaiting-placement" : "");
  ...
  slot.addEventListener("click", () => {
    if (!state.selectedItem) {
      if (!filled) flashToast("Click an item or block in the catalogue first, then click this slot.", true);
      return;
    }
    const rec = state.recipes.find(r => r.id === recipeId);
    if (!rec) return;
    placeIntoSlot(rec, slotKind, slotIndex ?? null, state.selectedItem);
  });
...
function renderSummary() {
  ...
  for (const rec of state.recipes) {
    ...
    card.addEventListener("click", () => { state.activeRecipeId = rec.id; state.selectedItem = null; renderAll(); });
...
function bedrockItemName(full_id) {
  // Catalogue is restricted to the minecraft: namespace, which maps 1:1 to Bedrock item/block ids
  // for the vast majority of vanilla content.
  return full_id;
}
